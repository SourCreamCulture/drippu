...
- Launch suyu and choose **Gamer** (or pass `-gamer`).
...
- **Empty state.** With no scan dirs and no NAND titles, `game_count` is `0`. Run `control-suyu screenshot --path "$SUYU_VERIFY_EVIDENCE_DIR/library-empty.png"`. The PNG shows heading `Library`, subtitle `Your collection, curated.`, stats `0 games in your library`, and `No games found.`
...
- **Confirm scan list.** Run `control-suyu call list_configured_game_dirs`. `directories` contains that absolute path with `deep_scan` true.
...
- **Proof.** Write `$SUYU_VERIFY_EVIDENCE_DIR/library.meta.json` with the last `get_ui_state` and `list_configured_game_dirs` bodies plus feature id `library-add-dir`. The empty-state PNG still shows suyu Gamer chrome.
